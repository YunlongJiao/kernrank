...
optR <- function(rold, zz, lam, data.info, perm.info)
{
  ...
  res <- try(nlm(f = negloglike_by_r, p = rold, zz = zz, lam = lam, data.info = data.info, perm.info = perm.info)$estimate, silent = TRUE)
  if (inherits(res, "try-error")) {
    warning("R not fully optimized due to nlm error")
    return(rold)
  } else {
    return(res)
  }
